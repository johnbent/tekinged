...
def insert_word(c,pword,pos,edef,stem=None,prompt=True):
  query = "insert into words_woleaian (pal,pos,eng,stem) values (%s,%s,%s,%s)"
  ...
  try:
    ...
    if (var != 'n'):
      ...
      rid=c.lastrowid
      if stem==None:
        query = "update words_woleaian set stem=id where id=%s"
        c.execute(query,(rid,))
...
def add_extra(c,wid,field,value):
  query = "update words_woleaian set %s=%%s where id=%%s" % field
  values = (value,wid)
...
def add_variant(c,var,pal):
  ...
  c.execute("""select id,pos,eng,stem,pdef from words_woleaian where pal like '%s'""" % pal)
  rows = c.fetchall()
  if (len(rows)>1):
...
def add_proverb(c,pal,eng,exp,wid):
  query = "insert into woleaian_proverbs (palauan,english,source,explanation,stem) values (%s,%s,%s,%s,%s)"
  values = (pal,eng,'Josephs',exp,wid)
  insert_example(c,query,values)

def add_example(c,pal,eng,wid):
  query = "insert into woleaian_examples (palauan,english,source,stem) values (%s,%s,%s,%s)"
...
def get_poses(c):
  try:
    ...
    get_poses.pos = [] 
    query="select distinct(pos) as pos from words_woleaian where !isnull(pos)";
    c.execute(query)
    rows = c.fetchall()
    for row in rows:
...
def get_branch_count(c,wid):
  q= "select count(*) as c from words_woleaian where stem=%s" % wid
...
def branch_to_root(c,branch,root):
  ...
  if (confirm == 'y'):
    ...
    query = "update words_woleaian set stem=id where id=%d" % branch
...
def really_add_word(c,pword,pos,edef,root=None,ignore_dups=False):
  ...
  for row in rows:
    if linked_without_eng(row,pos,root):
      query = "update words_woleaian set eng=%s where id=%s"
      values = (edef,row['id'])
      update_db(c,query,values,False)
      return (row['stem'],row['id'])
  ...
  if (answer == update and root != None):
    ...
    if rows[int(answer)]['eng'] is None or len(rows[int(answer)]['eng']) < 1:
      query = "update words_woleaian set stem=%s,eng=%s,pos=%s where id=%s"
      values = (root,edef,pos,rows[int(answer)]['id'])
    else:
      query = "update words_woleaian set stem=%s,pos=%s where id=%s"
    ...
    if r['group_sz'] > 1 and r['stem'] != root and r['id']==r['stem']:
      query = "update words_woleaian set stem=%s where stem=%s"
      values = (root,rows[int(answer)]['id'])
      update_db(c,query,values,True)
    return (root,rows[int(answer)]['id'])
  elif (answer < nword):
    query = "update words_woleaian set eng=%s,pos=%s where id=%s"
...
def add_link(c,root_words,a,b):
  ...
  for word in ( a , b ):
    if word.isdigit():  # user is specifying to link the previously added root word 
      ...
      print "\tUsing direct link word"
      stems.append(word)
      continue
    c.execute("""select id,pos,eng,stem,pdef from words_woleaian where pal like '%s' and id=stem""" % word)
    rows = c.fetchall()
    if (len(rows)>1):
...
def actually_link(c,a,b):
  ...
  update = """insert into woleaian_cf (a,b) values (%s,%s)""" % (lower,higher)
  try:
    #print update
    c.execute(update)
    if (c.rowcount):
